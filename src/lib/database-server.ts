// Server-side database client implementation
...
class QueryBuilder {
  ...
  like(column: string, pattern: string) {
    this.whereConditions.push({ condition: `${column} LIKE $${this.whereConditions.length + 1}`, value: pattern });
    return this;
  }

  // Add WHERE ILIKE condition (case insensitive)
  ilike(column: string, pattern: string) {
    this.whereConditions.push({ condition: `${column} ILIKE $${this.whereConditions.length + 1}`, value: pattern });
    return this;
  }

  // Add WHERE IN condition
  in(column: string, values: any[]) {
    this.whereConditions.push({ condition: `${column} = ANY($${this.whereConditions.length + 1})`, value: values });
    return this;
  }
    ...
    try {
      ...
      for (const condition of this.whereConditions) {
        if (condition.value !== null) {
          query = this.sql`${query} WHERE ${this.sql.unsafe(condition.condition.replace(/\$\d+/, '${condition.value}'))}`;
        } else {
